...
create or replace view lab_7_view as
select count(client.id), avg(amount) as average_amount, sum(amount) as total_amount, date
from sell
inner join client on sell.client_id = client.id
group by date;

select * from lab_7_view;
drop view lab_7_view;